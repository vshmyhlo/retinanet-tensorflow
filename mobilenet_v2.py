...
class DepthwiseConv2D(Network):
    def __init__(self, kernel_size, strides, padding, use_bias, name='separable_conv2d'):
...
class Bottleneck(Network):
    def __init__(self, filters, strides, expansion_factor, name='bottleneck'):
        super().__init__(name=name)
    ...
    def build(self, input_shape):
        self.expand_conv = Sequential([
            tf.layers.Conv2D(input_shape[3] * self._expansion_factor, 1, use_bias=False),
            tf.layers.BatchNormalization(),
            tf.nn.relu6
        ])

        self.depthwise_conv = Sequential([
            DepthwiseConv2D(3, strides=self._strides, padding='same', use_bias=False),
            tf.layers.BatchNormalization(),
            tf.nn.relu6
        ])

        self.linear_conv = Sequential([
            tf.layers.Conv2D(self._filters, 1, use_bias=False),
            tf.layers.BatchNormalization()
        ])
...
class MobileNetV2(Network):
    def __init__(self, name='mobilenet_v2'):
        ...
        self.input_conv = Sequential([
            tf.layers.Conv2D(32, 3, strides=2, padding='same', use_bias=False),
            tf.layers.BatchNormalization(),
            tf.nn.relu6
        ])

        self.bottleneck_1_1 = Bottleneck(16, expansion_factor=1, strides=1)

        self.bottleneck_2_1 = Bottleneck(24, expansion_factor=6, strides=2)
        self.bottleneck_2_2 = Bottleneck(24, expansion_factor=6, strides=1)

        self.bottleneck_3_1 = Bottleneck(32, expansion_factor=6, strides=2)
        self.bottleneck_3_2 = Bottleneck(32, expansion_factor=6, strides=1)
        self.bottleneck_3_3 = Bottleneck(32, expansion_factor=6, strides=1)

        self.bottleneck_4_1 = Bottleneck(64, expansion_factor=6, strides=2)
        self.bottleneck_4_2 = Bottleneck(64, expansion_factor=6, strides=1)
        self.bottleneck_4_3 = Bottleneck(64, expansion_factor=6, strides=1)
        self.bottleneck_4_4 = Bottleneck(64, expansion_factor=6, strides=1)

        self.bottleneck_5_1 = Bottleneck(96, expansion_factor=6, strides=1)
        self.bottleneck_5_2 = Bottleneck(96, expansion_factor=6, strides=1)
        self.bottleneck_5_3 = Bottleneck(96, expansion_factor=6, strides=1)

        self.bottleneck_6_1 = Bottleneck(160, expansion_factor=6, strides=2)
        self.bottleneck_6_2 = Bottleneck(160, expansion_factor=6, strides=1)
        self.bottleneck_6_3 = Bottleneck(160, expansion_factor=6, strides=1)

        self.bottleneck_7_1 = Bottleneck(320, expansion_factor=6, strides=1)

        self.output_conv = Sequential([
            tf.layers.Conv2D(32, 1, use_bias=False),
            tf.layers.BatchNormalization(),
            tf.nn.relu6
        ])
