  ...
  with tf.name_scope(name):
    ...
    with tf.control_dependencies(validate_output_shapes(true, pred)):
      for ct, rt, cp, rp in zip(*true, *pred):
        ...
        # regr_loss = tf.Print(regr_loss, [regr_loss])
        class_losses.append(class_loss)
        regr_losses.append(regr_loss)

    class_loss = sum(class_losses)  # sum over python list of values
    regr_loss = sum(regr_losses)  # sum over python list of values
